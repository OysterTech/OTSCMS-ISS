...
if(isset($_POST) && $_POST){
	...
	$itemSql="SELECT id,sex,group_name,name FROM item WHERE games_id=? AND scene=? AND order_index=? AND is_delete=0";
	...
	$scoreSql="SELECT a.*,b.short_name FROM score a,team b WHERE a.item_id=? AND a.team_id=b.id ORDER BY a.remark,LENGTH(a.score),a.score,a.run_group,a.runway";
...
<h3 style="font-weight:bold;text-align:center;color:orange;">成 绩 修 改</h3>
...
<form method="post">
	...
	<!-- 提交按钮 -->
	<center>
		<a class="btn btn-primary" style="width:48%" href="<?=ROOT_PATH.'admin/score.php?gamesId='.$gamesId;?>">< 返 回</a> <input type="submit" class="btn btn-success" style="width:48%" value="立 即 查 询 >">
	</center>
	<!-- ./提交按钮 -->
</form>
<!-- ./查询表单 -->
...
<tr>
	<td style="font-weight:bold;font-size:14px;color:blue;">
		<p id="rank_1_<?=$scoreQuery[0][$i]['id'];?>"><?=$scoreQuery[0][$i]['rank'];?></p>
		<input class="form-control" type="hidden" id="rank_2_<?=$scoreQuery[0][$i]['id'];?>" value="<?=$scoreQuery[0][$i]['rank'];?>">
	</td>
	<td><?=$scoreQuery[0][$i]['run_group'].'/'.$scoreQuery[0][$i]['runway'];?></td>
	<td><?=$scoreQuery[0][$i]['name'];?></td>
	<td><?=$scoreQuery[0][$i]['short_name'];?></td>
	<td>
		<p id="score_1_<?=$scoreQuery[0][$i]['id'];?>"><?=$scoreQuery[0][$i]['score'];?></p>
		<input class="form-control" type="hidden" id="score_2_<?=$scoreQuery[0][$i]['id'];?>" value="<?=$scoreQuery[0][$i]['score'];?>">
	</td>
	<td>
		<p id="point_1_<?=$scoreQuery[0][$i]['id'];?>"><?=$scoreQuery[0][$i]['point'];?></p>
		<input class="form-control" type="hidden" id="point_2_<?=$scoreQuery[0][$i]['id'];?>" value="<?=$scoreQuery[0][$i]['point'];?>">
</td>
	<td>
		<p id="remark_1_<?=$scoreQuery[0][$i]['id'];?>"><?=$scoreQuery[0][$i]['remark'];?></p>
		<input class="form-control" type="hidden" id="remark_2_<?=$scoreQuery[0][$i]['id'];?>" value="<?=$scoreQuery[0][$i]['remark'];?>">
	</td>
	<td>
		<button id="button_1_<?=$scoreQuery[0][$i]['id'];?>" onclick="readyUpdate('<?=$scoreQuery[0][$i]['id'];?>');" class="btn btn-primary">修改</button>
		<button id="button_2_<?=$scoreQuery[0][$i]['id'];?>" onclick="toUpdate('<?=$scoreQuery[0][$i]['id'];?>');" class="btn btn-success" style="display:none;">保存</button>
	</td>
</tr>
<?php } ?>
</table>
...
function readyUpdate(id){
	$("#rank_1_"+id).attr("style","display:none");
	$("#rank_2_"+id).attr("type","text");
	$("#score_1_"+id).attr("style","display:none");
	$("#score_2_"+id).attr("type","text");
	$("#point_1_"+id).attr("style","display:none");
	$("#point_2_"+id).attr("type","text");
	$("#remark_1_"+id).attr("style","display:none");
	$("#remark_2_"+id).attr("type","text");
...
function toUpdate(id){
	rank=$("#rank_2_"+id).val();
	score=$("#score_2_"+id).val();
	point=$("#point_2_"+id).val();
	remark=$("#remark_2_"+id).val();

	$("#rank_1_"+id).attr("style","");
	$("#rank_2_"+id).attr("type","hidden");
	$("#score_1_"+id).attr("style","");
	$("#score_2_"+id).attr("type","hidden");
	$("#point_1_"+id).attr("style","");
	$("#point_2_"+id).attr("type","hidden");
	$("#remark_1_"+id).attr("style","");
	$("#remark_2_"+id).attr("type","hidden");
	$("#button_1_"+id).attr("style","");
	$("#button_2_"+id).attr("style","display:none");

	$.ajax({
		url:"toUpdateScore.php",
		type:"POST",
		data:{"id":id,"rank":rank,"score":score,"point":point,"remark":remark},
		dataType:"JSON",
		success:function(ret){
			if(ret.code==200){
				alert("修改成功！");
				$("#rank_1_"+id).html(rank);
				$("#score_1_"+id).html(score);
				$("#point_1_"+id).html(point);
				$("#remark_1_"+id).html(remark);
			}else{
				console.log(ret);
				alert("修改失败！！！\n数据未被保存！");
			}
