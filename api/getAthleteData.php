...
if($type=='order'){
	...
	if($allGames==1){
		$scoreQuery=PDOQuery('SELECT a.name,a.score,a.rank,a.remark,b.sex,b.group_name,b.name AS item_name,c.name AS games_name,d.short_name FROM score a,item b,games c,team d WHERE a.name LIKE ? AND a.item_id=b.id AND b.games_id=c.id AND a.team_id=d.id ORDER BY c.start_date,a.rank,b.name',['%'.$name.'%'],[PDO::PARAM_STR]);
		...
		$scoreQuery=PDOQuery('SELECT a.name,a.score,a.rank,a.remark,b.sex,b.group_name,b.name AS item_name,c.short_name FROM score a,item b,team c WHERE a.name LIKE ? AND a.item_id=b.id AND a.team_id=c.id AND b.games_id=? ORDER BY b.scene,b.order_index',['%'.$name.'%',$gamesId],[PDO::PARAM_STR,PDO::PARAM_INT]);
	}
